...
def error_header(message, **kwargs):
    print("ERROR")
...
def for_each(alist, f, **kwargs):
    ...
    algorirhm can still feed it to f under the following conditions:
    ...
    **kwargs: keywaord arguments
    ...
    def an_argument(num, name):
        ...
        if not list_name in kwargs:
            if def_index < 0:
                error_header("Argument '{}' with no default value: impossible to call the function '{}'".format(name, f.__name__))
            call_args[name] = required_args.defaults[def_index]
            return True
        
        # Is the list really a list
        if not is_array(kwargs[list_name]):
            error_header("The argument '{}' must be a list of length {} to match the required argument '{}' in function '{}'".format(
