# ****************************************************************************************************
# Generated 2020-06-21

import numpy as np
from root import ArrayOf, Wrapper
...
class ArrayOfWEdge(ArrayOf):
    ...
    def bevel_weights(self, values): # array of float
        self._cache_bevel_weights = np.array(values)
        for obj, v in zip(self._array, values):
            obj.bevel_weight = v
    ...
    def creases(self, values): # array of float
        self._cache_creases = np.array(values)
        for obj, v in zip(self._array, values):
            obj.crease = v
    ...
    def hides(self, values): # array of bool
        self._cache_hides = np.array(values)
        for obj, v in zip(self._array, values):
            obj.hide = v
    ...
    def indices(self, values): # array of int
        self._cache_indices = np.array(values)
        for obj, v in zip(self._array, values):
            obj.index = v
    ...
    def is_looses(self, values): # array of bool
        self._cache_is_looses = np.array(values)
        for obj, v in zip(self._array, values):
            obj.is_loose = v
    ...
    def selects(self, values): # array of bool
        self._cache_selects = np.array(values)
        for obj, v in zip(self._array, values):
            obj.select = v
    ...
    def use_edge_sharps(self, values): # array of bool
        self._cache_use_edge_sharps = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_seams(self, values): # array of bool
        self._cache_use_seams = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_seam = v
...
class ArrayOfWLoop(ArrayOf):
    ...
    def bitangent_signs(self, values): # array of float
        self._cache_bitangent_signs = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def bitangents(self, values): # array of V3
        self._cache_bitangents = np.array(values)
        for obj, v in zip(self._array, values):
            obj.bitangent = v
    ...
    def edge_indices(self, values): # array of int
        self._cache_edge_indices = np.array(values)
        for obj, v in zip(self._array, values):
            obj.edge_index = v
    ...
    def normals(self, values): # array of V3
        self._cache_normals = np.array(values)
        for obj, v in zip(self._array, values):
            obj.normal = v
    ...
    def tangents(self, values): # array of V3
        self._cache_tangents = np.array(values)
        for obj, v in zip(self._array, values):
            obj.tangent = v
    ...
    def vertex_indices(self, values): # array of int
        self._cache_vertex_indices = np.array(values)
        for obj, v in zip(self._array, values):
            obj.vertex_index = v
...
class ArrayOfWPolygon(ArrayOf):
    ...
    def areas(self, values): # array of float
        self._cache_areas = np.array(values)
        for obj, v in zip(self._array, values):
            obj.area = v
    ...
    def centers(self, values): # array of V3
        self._cache_centers = np.array(values)
        for obj, v in zip(self._array, values):
            obj.center = v
    ...
    def loop_starts(self, values): # array of int
        self._cache_loop_starts = np.array(values)
        for obj, v in zip(self._array, values):
            obj.loop_start = v
    ...
    def loop_totals(self, values): # array of int
        self._cache_loop_totals = np.array(values)
        for obj, v in zip(self._array, values):
            obj.loop_total = v
    ...
    def material_indices(self, values): # array of int
        self._cache_material_indices = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_smooths(self, values): # array of bool
        self._cache_use_smooths = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_smooth = v
...
class ArrayOfWMeshVertex(ArrayOf):
    ...
    def cos(self, values): # array of V3
        self._cache_cos = np.array(values)
        for obj, v in zip(self._array, values):
            obj.co = v
    ...
    def undeformed_cos(self, values): # array of V3
        self._cache_undeformed_cos = np.array(values)
        for obj, v in zip(self._array, values):
            obj.undeformed_co = v
...
class ArrayOfWMesh(ArrayOf):
    ...
    def auto_smooth_angles(self, values): # array of float
        self._cache_auto_smooth_angles = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def auto_texspaces(self, values): # array of bool
        self._cache_auto_texspaces = np.array(values)
        for obj, v in zip(self._array, values):
            obj.auto_texspace = v
    ...
    def use_auto_smooths(self, values): # array of bool
        self._cache_use_auto_smooths = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_auto_texspaces(self, values): # array of bool
        self._cache_use_auto_texspaces = np.array(values)
        for obj, v in zip(self._array, values):
...
class ArrayOfWBezierSplinePoint(ArrayOf):
    ...
    def handle_left_types(self, values): # array of str
        self._cache_handle_left_types = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def handle_lefts(self, values): # array of V3
        self._cache_handle_lefts = np.array(values)
        for obj, v in zip(self._array, values):
            obj.handle_left = v
    ...
    def handle_right_types(self, values): # array of str
        self._cache_handle_right_types = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def handle_rights(self, values): # array of V3
        self._cache_handle_rights = np.array(values)
        for obj, v in zip(self._array, values):
            obj.handle_right = v
    ...
    def radius_s(self, values): # array of float
        self._cache_radius_s = np.array(values)
        for obj, v in zip(self._array, values):
            obj.radius = v
    ...
    def tilts(self, values): # array of int
        self._cache_tilts = np.array(values)
        for obj, v in zip(self._array, values):
            obj.tilt = v
    ...
    def weight_softbodys(self, values): # array of float
        self._cache_weight_softbodys = np.array(values)
        for obj, v in zip(self._array, values):
...
class ArrayOfWSplinePoint(ArrayOf):
    ...
    def weights(self, values): # array of float
        self._cache_weights = np.array(values)
        for obj, v in zip(self._array, values):
            obj.weight = v
...
class ArrayOfWSpline(ArrayOf):
    ...
    def character_indices(self, values): # array of int
        self._cache_character_indices = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def order_us(self, values): # array of int
        self._cache_order_us = np.array(values)
        for obj, v in zip(self._array, values):
            obj.order_u = v
    ...
    def order_vs(self, values): # array of int
        self._cache_order_vs = np.array(values)
        for obj, v in zip(self._array, values):
            obj.order_v = v
    ...
    def point_count_us(self, values): # array of int
        self._cache_point_count_us = np.array(values)
        for obj, v in zip(self._array, values):
            obj.point_count_u = v
    ...
    def point_count_vs(self, values): # array of int
        self._cache_point_count_vs = np.array(values)
        for obj, v in zip(self._array, values):
            obj.point_count_v = v
    ...
    def radius_interpolations(self, values): # array of str
        self._cache_radius_interpolations = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def resolution_us(self, values): # array of int
        self._cache_resolution_us = np.array(values)
        for obj, v in zip(self._array, values):
            obj.resolution_u = v
    ...
    def resolution_vs(self, values): # array of int
        self._cache_resolution_vs = np.array(values)
        for obj, v in zip(self._array, values):
            obj.resolution_v = v
    ...
    def types(self, values): # array of str
        self._cache_types = np.array(values)
        for obj, v in zip(self._array, values):
            obj.type = v
    ...
    def use_bezier_us(self, values): # array of bool
        self._cache_use_bezier_us = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_bezier_u = v
    ...
    def use_bezier_vs(self, values): # array of bool
        self._cache_use_bezier_vs = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_bezier_v = v
    ...
    def use_cyclic_us(self, values): # array of bool
        self._cache_use_cyclic_us = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_cyclic_u = v
    ...
    def use_cyclic_vs(self, values): # array of bool
        self._cache_use_cyclic_vs = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_cyclic_v = v
    ...
    def use_endpoint_us(self, values): # array of bool
        self._cache_use_endpoint_us = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_endpoint_vs(self, values): # array of bool
        self._cache_use_endpoint_vs = np.array(values)
        for obj, v in zip(self._array, values):
...
class ArrayOfWCurve(ArrayOf):
    ...
    def bevel_depths(self, values): # array of float
        self._cache_bevel_depths = np.array(values)
        for obj, v in zip(self._array, values):
            obj.bevel_depth = v
    ...
    def bevel_factor_ends(self, values): # array of float
        self._cache_bevel_factor_ends = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def bevel_factor_starts(self, values): # array of float
        self._cache_bevel_factor_starts = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def bevel_resolutions(self, values): # array of int
        self._cache_bevel_resolutions = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def dimensions_s(self, values): # array of str
        self._cache_dimensions_s = np.array(values)
        for obj, v in zip(self._array, values):
            obj.dimensions = v
    ...
    def eval_times(self, values): # array of float
        self._cache_eval_times = np.array(values)
        for obj, v in zip(self._array, values):
            obj.eval_time = v
    ...
    def extrudes(self, values): # array of float
        self._cache_extrudes = np.array(values)
        for obj, v in zip(self._array, values):
            obj.extrude = v
    ...
    def fill_modes(self, values): # array of str
        self._cache_fill_modes = np.array(values)
        for obj, v in zip(self._array, values):
            obj.fill_mode = v
    ...
    def offsets(self, values): # array of float
        self._cache_offsets = np.array(values)
        for obj, v in zip(self._array, values):
            obj.offset = v
    ...
    def path_durations(self, values): # array of int
        self._cache_path_durations = np.array(values)
        for obj, v in zip(self._array, values):
            obj.path_duration = v
    ...
    def render_resolution_us(self, values): # array of int
        self._cache_render_resolution_us = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def render_resolution_vs(self, values): # array of int
        self._cache_render_resolution_vs = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def twist_smooths(self, values): # array of float
        self._cache_twist_smooths = np.array(values)
        for obj, v in zip(self._array, values):
            obj.twist_smooth = v
    ...
    def use_deform_bounds_s(self, values): # array of bool
        self._cache_use_deform_bounds_s = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_fill_caps_s(self, values): # array of bool
        self._cache_use_fill_caps_s = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_fill_caps = v
    ...
    def use_fill_deforms(self, values): # array of bool
        self._cache_use_fill_deforms = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_map_tapers(self, values): # array of bool
        self._cache_use_map_tapers = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_map_taper = v
    ...
    def use_path_follows(self, values): # array of bool
        self._cache_use_path_follows = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_paths(self, values): # array of bool
        self._cache_use_paths = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_path = v
    ...
    def use_radius_s(self, values): # array of bool
        self._cache_use_radius_s = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_radius = v
    ...
    def use_stretchs(self, values): # array of bool
        self._cache_use_stretchs = np.array(values)
        for obj, v in zip(self._array, values):
            obj.use_stretch = v
...
class ArrayOfWObject(ArrayOf):
    ...
    def active_material_indices(self, values): # array of int
        self._cache_active_material_indices = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def active_shape_key_indices(self, values): # array of int
        self._cache_active_shape_key_indices = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def colors(self, values): # array of V4
        self._cache_colors = np.array(values)
        for obj, v in zip(self._array, values):
            obj.color = v
    ...
    def delta_locations(self, values): # array of V3
        self._cache_delta_locations = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def delta_rotation_eulers(self, values): # array of V3
        self._cache_delta_rotation_eulers = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def delta_rotation_quaternions(self, values): # array of V4
        self._cache_delta_rotation_quaternions = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def delta_scales(self, values): # array of V3
        self._cache_delta_scales = np.array(values)
        for obj, v in zip(self._array, values):
            obj.delta_scale = v
    ...
    def empty_display_sizes(self, values): # array of float
        self._cache_empty_display_sizes = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def empty_image_offsets(self, values): # array of V2
        self._cache_empty_image_offsets = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def hide_renders(self, values): # array of bool
        self._cache_hide_renders = np.array(values)
        for obj, v in zip(self._array, values):
            obj.hide_render = v
    ...
    def hide_selects(self, values): # array of bool
        self._cache_hide_selects = np.array(values)
        for obj, v in zip(self._array, values):
            obj.hide_select = v
    ...
    def hide_viewports(self, values): # array of bool
        self._cache_hide_viewports = np.array(values)
        for obj, v in zip(self._array, values):
            obj.hide_viewport = v
    ...
    def instance_faces_scales(self, values): # array of float
        self._cache_instance_faces_scales = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def locations(self, values): # array of V3
        self._cache_locations = np.array(values)
        for obj, v in zip(self._array, values):
            obj.location = v
    ...
    def lock_scales(self, values): # array of bool
        self._cache_lock_scales = np.array(values)
        for obj, v in zip(self._array, values):
            obj.lock_scale = v
    ...
    def pass_indices(self, values): # array of int
        self._cache_pass_indices = np.array(values)
        for obj, v in zip(self._array, values):
            obj.pass_index = v
    ...
    def rotation_eulers(self, values): # array of V3
        self._cache_rotation_eulers = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def rotation_modes(self, values): # array of str
        self._cache_rotation_modes = np.array(values)
        for obj, v in zip(self._array, values):
            obj.rotation_mode = v
    ...
    def rotation_quaternions(self, values): # array of V4
        self._cache_rotation_quaternions = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def scales(self, values): # array of V3
        self._cache_scales = np.array(values)
        for obj, v in zip(self._array, values):
            obj.scale = v
    ...
    def show_all_edges_s(self, values): # array of bool
        self._cache_show_all_edges_s = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_axis_s(self, values): # array of bool
        self._cache_show_axis_s = np.array(values)
        for obj, v in zip(self._array, values):
            obj.show_axis = v
    ...
    def show_bounds_s(self, values): # array of bool
        self._cache_show_bounds_s = np.array(values)
        for obj, v in zip(self._array, values):
            obj.show_bounds = v
    ...
    def show_empty_image_only_axis_aligneds(self, values): # array of bool
        self._cache_show_empty_image_only_axis_aligneds = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_empty_image_orthographics(self, values): # array of bool
        self._cache_show_empty_image_orthographics = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_empty_image_perspectives(self, values): # array of bool
        self._cache_show_empty_image_perspectives = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_in_fronts(self, values): # array of bool
        self._cache_show_in_fronts = np.array(values)
        for obj, v in zip(self._array, values):
            obj.show_in_front = v
    ...
    def show_instancer_for_renders(self, values): # array of bool
        self._cache_show_instancer_for_renders = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_instancer_for_viewports(self, values): # array of bool
        self._cache_show_instancer_for_viewports = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_names(self, values): # array of bool
        self._cache_show_names = np.array(values)
        for obj, v in zip(self._array, values):
            obj.show_name = v
    ...
    def show_only_shape_keys(self, values): # array of bool
        self._cache_show_only_shape_keys = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_texture_spaces(self, values): # array of bool
        self._cache_show_texture_spaces = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_transparents(self, values): # array of bool
        self._cache_show_transparents = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def show_wires(self, values): # array of bool
        self._cache_show_wires = np.array(values)
        for obj, v in zip(self._array, values):
            obj.show_wire = v
    ...
    def track_axis_s(self, values): # array of str
        self._cache_track_axis_s = np.array(values)
        for obj, v in zip(self._array, values):
            obj.track_axis = v
    ...
    def up_axis_s(self, values): # array of str
        self._cache_up_axis_s = np.array(values)
        for obj, v in zip(self._array, values):
            obj.up_axis = v
    ...
    def use_empty_image_alphas(self, values): # array of bool
        self._cache_use_empty_image_alphas = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_instance_faces_scales(self, values): # array of bool
        self._cache_use_instance_faces_scales = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_instance_vertices_rotations(self, values): # array of bool
        self._cache_use_instance_vertices_rotations = np.array(values)
        for obj, v in zip(self._array, values):
    ...
    def use_shape_key_edit_modes(self, values): # array of bool
        self._cache_use_shape_key_edit_modes = np.array(values)
        for obj, v in zip(self._array, values):
